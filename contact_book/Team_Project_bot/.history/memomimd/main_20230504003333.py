...
def greet(*args):
    if languages:
        return "Hello, I am your personal MemoMind 1.0.0 bot assistant. How can I help?"
    else:
        return (
...
def load_data(book1: AddressBook, notebook: NotePad):
    ...
    with open(os.path.join("memomimd", "config.JSON")) as cfg:
        ...
        dir_path = os.path.dirname("\memomimd")
        print(dir_path)
        db_file_name = os.path.join("memomimd", cfg_data["PhoneBookFile"])
        note_file_name = os.path.join("memomimd", cfg_data["NoteBookFile"])
        PAGE = cfg_data["Page"]
