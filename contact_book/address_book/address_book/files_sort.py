    ...
    for el in os.listdir(path):  # перебираємо всі елементи в структурі (теки, файли)
        if os.path.isdir(path + '\\' + el):  # якщо елемент є тека
            try:
                ...
                logging.info("directory '%s' has been removed successfully" % (path + '\\' + el))
                delete_empty_folders(path)
            except OSError:
                print(Fore.RED + "  directory '%s' can not be removed" % (path + '\\' + el))
                logging.info("directory '%s' can not be removed" % (path + '\\' + el))
        ...
        if user_input == '1':
            ...
            try:
                if os.path.exists(path):
                    ...
                    logging.info(f'path {path} is not found, try again')
                    input(Fore.YELLOW + '\n  press Enter to continue')
