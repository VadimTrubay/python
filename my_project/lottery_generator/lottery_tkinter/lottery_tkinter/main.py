...
def main():
    ...
    def get_balls():
        ...
        for _ in numbers[:selected_ball]:
            ...
            b = Button(text=f"{num}", bd=2, font=('Arial', 20, 'bold'), fg='#446600')
            b.grid(row=7, column=column, padx=5, pady=5)
            column += 1
            new_list.append(num)
            numbers.remove(num)
    ...
    def sort_balls():
        ...
        column = 0
        for num in new_list:
            b = Button(text=f"{num}", bd=2, font=('Arial', 20, 'bold'), fg='#006600')
            b.grid(row=9, column=column, padx=5, pady=5)
            column += 1
            root.update()
            time.sleep(1)
        try_again()
    ...
    label = Label(text="lottery_tkinter")
    label.grid(row=0, column=0, padx=10, pady=10)

    button_start = Button(text="Start", command=get_balls, bd=5, font=('Arial', 10, 'bold'))
    button_start.grid(row=1, column=0, padx=10, pady=10)

    button_start = Button(text=" Sort ", command=sort_balls, bd=5, font=('Arial', 10, 'bold'))
    button_start.grid(row=1, column=1, padx=10, pady=10)

    button_clear = Button(text="Clear", command=clear, bd=5, font=('Arial', 10, 'bold'))
    button_clear.grid(row=1, column=2, padx=10, pady=10)

    button_exiting = Button(text=" Exit ", command=exiting, bd=5, font=('Arial', 10, 'bold'))
