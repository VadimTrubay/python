...
class NoteBook(UserList):
    ...
    def add(self, record):
        contact = {"title": record.title, "note": record.note, "tag": record.tag}
        self.data.append(contact)
        print(Fore.RED + f"  record {record.title} added")
        logging.info(f"record {record.title} added")
    ...
    def edit_note(self, title, parameter, new_value):
        for key in self.data:
            if key["title"] == title:
                ...
                logging.info(f"note {title} edited")
                break
            else:
                continue
        ...
        for key in self.data:
            if key["title"] == name:
                ...
                if del_contact == "y":
                    ...
                    logging.info(f'note {key["title"]} deleted')
                    break
                else:
                    break
        else:
    ...
    def load(self, file_name):
        try:
            ...
            if empty_ness.st_size != 0:
                ...
                logging.info("note book loaded")
        except FileNotFoundError:
            with open(file_name, "wb+") as file:
                pickle.dump(self.data, file)
            print(Fore.RED + "\n  note book created")
            logging.info("note book created")
    ...
    def __init__(self, value=""):
        while True:
            ...
            try:
                ...
                logging.info(f"incorrect value")
            ...
            if command == "1":
                ...
                while True:
                    ...
                    if clear_all == "y":
                        ...
                        logging.info("note book cleared")
                        break
                    else:
                        break
