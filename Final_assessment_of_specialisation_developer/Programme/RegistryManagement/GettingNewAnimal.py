...
class GettingAnimal(Animals):
    """Класс новое животное"""

    def __init__(self, type_animal, name, birth_date, command):
    ...
    def domestic_new_animal(self, config, count):
        ...
        try:
            with connect(
                ...
                VALUES 
                ({self.get_type_animal()}, 1, "{self.get_name()}", "{self.get_birth_date()}", "{self.get_command()}");
                """
    ...
    def pack_new_animal(self, config, count):
        ...
        try:
            with connect(
                ...
                VALUES 
                ({self.get_type_animal()}, 2, "{self.get_name()}", "{self.get_birth_date()}", "{self.get_command()}");
                """
                with connection.cursor() as cursor:
                    cursor.execute(insert_type_animal_query)
                    connection.commit()
