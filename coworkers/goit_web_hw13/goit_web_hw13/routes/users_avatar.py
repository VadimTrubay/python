...
async def update_avatar_user(file: UploadFile = File(),
    ...
    goit_web_hw13_url = cloudinary.CloudinaryImage(f'NotesApp/{current_user.username}')\
                        .build_url(width=250, height=250, crop='fill', version=r.get('version'))
    user = await repository_auths.update_avatar(current_user.email, goit_web_hw13_url, db)
    return user
